...
function doRectanglesOverlap(rect1, rect2) {
  const a = rect1.left;
  const b = rect1.top;
  const c = rect1.left + rect1.width;
  const d = rect1.top + rect1.height;
  const e = rect2.left;
  const f = rect2.top;
  const g = rect2.left + rect2.width;
  const h = rect2.top + rect2.height;
...
function getDigitalRoot(num) {
  ...
  const result = number.reduce((summa, value) => summa + value, 0);
...
function isBracketsBalanced(str) {
  let strnew = str;
  while (true) {
    const result = strnew.replace(/\(\)|\[\]|\{\}|<>/, '');
    if (result === strnew) return result.length === 0;
    strnew = result;
  }
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    if (X && position[0][i] !== undefined) {
      result.push(position[0][i]);
    }
    if (Y && position[i][0] !== undefined) {
